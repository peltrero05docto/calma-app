...
export const MentalHealthJournal: React.FC<Props> = ({ userName, addPoints, onSave }) => {
  ...
  const handleMoodSelect = async (moodName: string, emoji: string) => {
    ...
    try {
      ...
      setAiResponse("¡Eres increíble, no lo olvides!");
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto flex flex-col items-center animate-fade-in">
      <h2 className="text-3xl font-bold text-calm-800 mb-2 text-center">¿Cómo va todo hoy?</h2>
      <p className="text-gray-600 mb-8 text-center">Cuéntale a Calma cómo te sientes.</p>
      ...
      {!selectedMood ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 w-full">
          {MOODS.map((m) => (
            <button
              ...
              className={`${m.color} p-6 rounded-3xl flex flex-col items-center gap-2 hover:scale-105 transition-all shadow-sm border-2 border-transparent hover:border-white active:scale-95`}
            >
        ...
        <div className="bg-white p-8 rounded-3xl shadow-xl w-full text-center border border-calm-100">
          ...
          <div className="bg-calm-50 p-6 rounded-2xl relative min-h-[80px] flex items-center justify-center">
            {loading ? (
              ...
              <p className="text-lg text-calm-900 font-medium italic leading-relaxed">"{aiResponse}"</p>
            )}
            <div className="absolute -top-3 left-6 bg-calm-600 text-white text-[10px] uppercase font-black px-2 py-1 rounded shadow-sm">CALMA DICE</div>
          </div>

          <button
            onClick={() => setSelectedMood(null)}
            className="mt-8 text-calm-600 font-bold hover:text-calm-800 transition-colors"
          >
            ← Cambiar estado
          </button>
        </div>
