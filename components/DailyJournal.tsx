...
export const DailyJournal: React.FC<Props> = ({ userName, addPoints, onSave }) => {
  ...
  const handleMoodSelect = async (moodName: string, emoji: string) => {
    ...
    try {
      ...
      setAiResponse("¡Ánimo, tú puedes con todo!");
    } finally {
      setLoading(false);
    }
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 w-full">
          {MOODS.map((m) => (
            <button
              ...
              className={`${m.color} p-6 rounded-3xl flex flex-col items-center gap-2 hover:scale-105 transition-transform shadow-sm border-2 border-transparent hover:border-white active:scale-95`}
            >
