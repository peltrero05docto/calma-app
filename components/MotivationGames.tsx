...
export const MotivationGames: React.FC<Props> = ({ addPoints }) => {
  ...
  const startScrambleGame = async () => {
    ...
    try {
      ...
      if (!quote) throw new Error("No hay frase");
      setTargetQuote(quote);
      setTargetHint(hint);
      const words = quote.split(' ').filter(w => w.length > 0).map((w, i) => ({ id: Math.random() + i, text: w }));
      setScrambledWords([...words].sort(() => Math.random() - 0.5));
      setGameStatus('playing');
    } catch (e) {
      console.error("Error cargando reto:", e);
      setGameStatus('playing');
    }
  };
  ...
  const resetScramble = () => {
     setUserOrder([]);
     const words = targetQuote.split(' ').filter(w => w.length > 0).map((w, i) => ({ id: Math.random() + i, text: w }));
     setScrambledWords([...words].sort(() => Math.random() - 0.5));
     setGameStatus('playing');
  };
      ...
      {activeTab === 'scramble' && (
        <div className="bg-white p-6 sm:p-8 rounded-[2.5rem] shadow-xl border-b-8 border-calm-100">
          <div className="text-center mb-8">
            <h3 className="text-xl font-black text-gray-800">Encuentra el mensaje positivo</h3>
          ...
          {gameStatus === 'loading' ? (
            <div className="flex flex-col items-center justify-center py-12 gap-4">
              ...
              <p className="text-calm-600 font-bold animate-pulse">Generando nuevo reto...</p>
            </div>
          ) : (
            <>
              <div className="min-h-[140px] bg-calm-50 rounded-[2rem] border-4 border-dashed border-calm-200 mb-8 p-6 flex flex-wrap gap-3 justify-center items-center">
                {userOrder.length === 0 && (
                  <div className="text-gray-300 select-none text-center">
                    ...
                    <p className="font-bold italic">Toca las palabras de abajo</p>
                  </div>
                )}
                {userOrder.map((w, idx) => (
                  <span key={`user-${w.id}-${idx}`} className="bg-calm-600 text-white px-5 py-3 rounded-2xl font-black shadow-lg animate-fade-in border-b-4 border-calm-800">
                    {w.text}
                  </span>
                ))}
              </div>
              ...
              <div className="bg-blue-50/50 p-4 rounded-2xl border border-blue-100 mb-8">
                {showHint ? (
                  ...
                  <button onClick={() => setShowHint(true)} className="w-full text-blue-600 font-black text-xs uppercase tracking-widest hover:underline">¿Necesitas una pista? 🔍</button>
                )}
              </div>

              <div className="flex justify-center items-center gap-4">
                <button onClick={resetScramble} className="bg-gray-100 text-gray-500 px-6 py-3 rounded-2xl font-bold hover:bg-gray-200 active:scale-90">Reiniciar</button>
              ...
              {gameStatus === 'won' && (
                <div className="mt-8 p-6 bg-green-50 text-green-800 rounded-3xl text-center animate-bounce border-2 border-green-200 shadow-xl">
                  ...
                  <p className="text-xl font-black">¡CHIDÍSIMO!</p>
            ...
            <>
              ...
              <div className="space-y-3">
                ...
                <textarea 
                  ...
                  placeholder="Ej: 'Esto es difícil ahora, pero sé que puedo aprender...'"
              ...
              {reframeResult && (
                <div className={`p-6 rounded-3xl border-2 animate-fade-in shadow-lg ${reframeResult.score >= 7 ? 'bg-green-50 border-green-200' : 'bg-yellow-50 border-yellow-200'}`}>
                  <div className="flex justify-between items-center mb-3">
                    <span className="font-black text-lg uppercase">Puntaje: {reframeResult.score}/10</span>
